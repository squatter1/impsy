...
Due to the amount of meta-variables controlling the system, it was not possible to perform rigorous testing on the best possible combination of them. Instead, a series of smaller tests were performed on each parameter individually, some brief evaluations of multiple possible values, and other based on the feel of the system when performing with it. We will go over each of these important parameters here in turn, excluding those relevant only to specific heuristics and hence mentioned in those sections. This serves not only to state the values used in the final system, but also to provide an overview of each of these constants and their relevance to the system.

TODO: write this when rereading through the section to include anything mentioned. Also then look through code for any parameters. Also include weights of component heurstics
